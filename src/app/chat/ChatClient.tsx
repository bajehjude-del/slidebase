...
function buildInsights(rules: Rule[]): InsightCard[] {
  ...
  if (saveRules.length > 0) {
    cards.push({
      ...
      body: `You have ${saveRules.length} active saving rule${saveRules.length > 1 ? "s" : ""}. Slidebase is watching every payment and setting aside your target amount automatically.`,
  ...
  cards.push({
    ...
    title: "Weekly tip",
    body: "Consider setting a monthly spending buffer rule — if your balance drops below a threshold, Slidebase can alert you and pause non-essential outflows.",
      ...
      {activated && (
        <div className="px-4 pb-4">
          <div className="flex items-center gap-2 py-2.5 px-4 rounded-xl bg-green-500/10 border border-green-500/20 text-green-400 text-sm font-medium">
            ...
            Rule activated — Slidebase is watching
          </div>
        </div>
      )}
    </motion.div>
...
export default function ChatClient({ initialRules }: { initialRules: Rule[] }) {
  ...
  const handleActivate = async (rule: ParsedRule) => {
    ...
    if (res.ok) {
      ...
      setToast("Rule activated — Slidebase is watching 👁");
      setMessages(prev => [...prev, {
        id: crypto.randomUUID(), role: "assistant",
        content: "✅ Rule activated! Slidebase will now monitor your wallet. Want to create another?",
        isConfirmation: true,
      }]);
    } else {
      ...
      <div className="flex-shrink-0 px-4 md:px-6 py-4 md:py-5 border-b border-white/[0.06] bg-black/50 backdrop-blur-md">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-semibold text-white">Slidebase AI</h1>
